...
class DiffDrive:
	def __init__(self):
		...
		self.wheel_pub = rospy.Publisher("motor_cmd", JointState, queue_size=1)
	...
	def velocity(self, msg):
		...
		if msg.angular.z != 0 or msg.linear.x != 0:
			...
			#consistency with diff_drive_controller, which does not reverse the same way
			if msg.linear.x < 0:
		...
		left_effort = clamp(left_vel / self.SPEED_SCALE, -1.0, 1.0)
		right_effort = clamp(right_vel / self.SPEED_SCALE, -1.0, 1.0)

		state = JointState()
		state.name = ["left_thrust", "right_thrust"]
		state.effort = [left_effort, right_effort]
